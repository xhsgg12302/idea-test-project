...
public class JmsConsumer {
    public static void main(String[] args) throws Exception {
       ...
       // MessageProducer messageProducer; // 消息消费者
        ...
        messageConsumer = session.createDurableSubscriber((Topic)destination,"name_1"); //持久订阅
        ...
        messageConsumer.setMessageListener(new MessageListener() {
            ...
            public void onMessage(Message message) {
                ...
                try {
                    MessageProducer producer = session.createProducer(tm.getJMSReplyTo());

                    System.out.println(tm.getText() + "  \t"+tm.getJMSReplyTo().toString());
                    TextMessage textMessage = session.createTextMessage("consumer:"+tm.getText() + "  \t"+tm.getJMSReplyTo().toString());
